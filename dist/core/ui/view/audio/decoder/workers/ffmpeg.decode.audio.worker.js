...
self.onmessage = function (e) {
    ...
    checkForInit();
    var decodedFrame = decode(pktData);
...
function checkForInit() {
    ...
    if (initBuffer.length > 0) {
        ...
        for (var _i = 0, initBuffer_1 = initBuffer; _i < initBuffer_1.length; _i++) {
            var frameData = initBuffer_1[_i];
            var decodedFrame = decode(frameData);
            sendBack(decodedFrame);
        }
        initBuffer = [];
    }
...
function init(codec) {
    ...
    // find audio decoder
    console.log("Init Ffmpeg audio decoder using ".concat(codec, " codec"));
...
function decode(pktData) {
    try {
        ...
        var type = instance.getValue(self.got_frame, 'i8');
        if (type === 0) {
...
//# sourceMappingURL=ffmpeg.decode.audio.worker.js.map