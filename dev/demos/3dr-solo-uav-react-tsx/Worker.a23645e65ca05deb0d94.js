...
class DataSynchronizerAlgo {
  constructor(dataSources) {
    let timerResolution = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 5;
    this.dataSourceMap = {};
  ...
  removeDataSource(dataSourceId) {
    ...
    console.log("Removed datasource ".concat(dataSourceId, " from algo.."));
  }
  ...
  processData() {
    ...
    console.warn("Started Algorithm ".concat(this.constructor.name, " with  tsRef=").concat(new Date(tsRef).toISOString()));
  }
  /**
...
class DataSynchronizerAlgoReplay extends timesync_DataSynchronizerAlgo {
  constructor(dataSources) {
    let replaySpeed = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
    let startTimestamp = arguments.length > 2 ? arguments[2] : undefined;
    let endTimestamp = arguments.length > 3 ? arguments[3] : undefined;
    let timerResolution = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 5;
    let version = arguments.length > 5 ? arguments[5] : undefined;
...
;// CONCATENATED MODULE: ./node_modules/babel-loader/lib/index.js??ruleSet[1].rules[4].use[0]!./node_modules/@open-wc/webpack-import-meta-loader/webpack-import-meta-loader.js!../../../source/core/timesync/replay/DataSynchronizer.replay.worker.js
...
async function handleMessage(event) {
  ...
  try {
    if (event.data.message === 'init') {
      ...
      dataSynchronizerAlgo = new DataSynchronizerAlgo_replay(event.data.dataSources, replaySpeed, startTimestamp, endTimestamp, event.data.timerResolution, version);
      ...
      setTimeRange(event.data.startTimestamp, event.data.endTimestamp, event.data.mode, event.data.replaySpeed, event.data.version, event.data.dataSources);
...
function setTimeRange(startTimestamp, endTimestamp, mode, replaySpeed, newVersion, dsArray) {
  DataSynchronizer_replay_worker_reset();
  version = newVersion;
  dataSynchronizerAlgo = new DataSynchronizerAlgo_replay(dsArray, replaySpeed, startTimestamp, endTimestamp, timerResolution, version);
...
function initBroadcastChannel(dataTopic, timeTopic) {
  ...
  dataSynchronizerBroadCastChannel.onmessage = async event => {
    if (event.data.type === EventType.DATA) {
      ...
      dataSynchronizerAlgo.setStatus(dataSourceId, event.data.status); // bubble the message
...
function addDataSource(dataSource) {
  dataSynchronizerAlgo.addDataSource(dataSource); // create a BC to push back the synchronized data into the DATA Stream.
...
function removeDataSource(dataSourceId) {
  dataSynchronizerAlgo.removeDataSource(dataSourceId); // create a BC to push back the synchronized data into the DATA Stream.
...
async function onEnd() {
  ...
  clearInterval(masterTimeInterval);
  masterTimeInterval = undefined; // end at this time
  ...
  if (!isDefined(masterTimeInterval)) {
    masterTimeInterval = setInterval(() => {
      ...
      if (isDefined(lastData)) {
        ...
        if (cTime !== -1 && lastTime === -1 || lastTime !== -1 && cTime !== lastTime) {
          // does not send the same data twice
          timeBroadcastChannel.postMessage({
            timestamp: cTime,
            dataSourceId: cId,
