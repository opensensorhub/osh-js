...
;// CONCATENATED MODULE: ../../../source/core/timesync/rt/DataSynchronizerAlgo.realtime.js
...
;// CONCATENATED MODULE: ./node_modules/babel-loader/lib/index.js??ruleSet[1].rules[4].use[0]!./node_modules/@open-wc/webpack-import-meta-loader/webpack-import-meta-loader.js!../../../source/core/timesync/rt/DataSynchronizer.realtime.worker.js
...
async function handleMessage(event) {
  ...
  try {
    if (event.data.message === 'init') {
      ...
      dataSynchronizerAlgo = new DataSynchronizerAlgo_realtime(event.data.dataSources, event.data.replaySpeed, event.data.timerResolution);
...
function DataSynchronizer_realtime_worker_reset() {
  ...
  timeBroadcastChannel.postMessage({
    type: EventType_EventType.TIME_CHANGED
  });
  timeBroadcastChannel.postMessage({
    type: EventType_EventType.CLOSED
  });
  dataSynchronizerBroadCastChannel.postMessage({
    type: EventType_EventType.STATUS,
    status: 'not_running'
  });
}
...
function initBroadcastChannel(dataTopic, timeTopic) {
  ...
  dataSynchronizerBroadCastChannel.onmessage = event => {
    ...
    if (event.data.type === EventType_EventType.DATA) {
      dataSynchronizerAlgo.push(event.data.dataSourceId, event.data.values);
    } else if (event.data.type === EventType_EventType.STATUS) {
  ...
  if (!isDefined(masterTimeInterval)) {
    masterTimeInterval = setInterval(() => {
      ...
      if (isDefined(masterTime)) {
        timeBroadcastChannel.postMessage({
          timestamp: masterTime,
          type: EventType_EventType.MASTER_TIME
        });
      }
      ...
      if (isDefined(lastData)) {
        ...
        if (cTime !== -1 && lastTime === -1 || lastTime !== -1 && cTime !== lastTime) {
          ...
          timeBroadcastChannel.postMessage({
            timestamp: cTime,
            dataSourceId: cId,
            type: EventType_EventType.LAST_TIME
          });
        }
