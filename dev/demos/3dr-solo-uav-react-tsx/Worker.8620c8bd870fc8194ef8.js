...
class DataSynchronizerAlgo_DataSynchronizerAlgo {
  ...
  close() {
    if (Utils_isDefined(this.interval)) {
...
class DataSynchronizerAlgoReplay extends (/* unused pure expression or super */ null && (DataSynchronizerAlgo)) {
...
/* harmony default export */ var DataSynchronizerAlgo_replay = ((/* unused pure expression or super */ null && (DataSynchronizerAlgoReplay)));
...
class DataSynchronizerAlgoRealtime extends timesync_DataSynchronizerAlgo {
  ...
  addDataSource(dataSource) {
    this.dataSourceMap[dataSource.id] = {
      ...
      name: dataSource.name || dataSource.id,
      latency: 0,
      status: Status_Status.DISCONNECTED,
      //MEANING Enabled, 0 = Disabled
      version: undefined
    };
    this.datasources.push(dataSource);
  ...
  checkVersion(datasource, dataBlock) {
    if (datasource.status !== Status_Status.DISCONNECTED) {
      return true;
    } else if (datasource.status === Status_Status.DISCONNECTED && datasource.version !== dataBlock.version) {
      return false;
    }
  }
  /**
  ...
  resetDataSource(datasourceId) {
    ...
    currentDs.latency = 0;
    currentDs.status = Status_Status.DISCONNECTED;
    currentDs.version = undefined;
  }
        ...
        try {
            ...
            if (event.data.message === 'init') {
                ...
                data = {
                    ...
                    data: Utils_isDefined(masterTimeInterval) && Utils_isDefined(dataSynchronizerAlgo) && Utils_isDefined(dataSynchronizerAlgo.interval)
                };
...
function checkMasterTime() {
    if(!Utils_isDefined(masterTimeInterval)) {
        startMasterTimeInterval(masterTimeRefreshRate);
    }
}
