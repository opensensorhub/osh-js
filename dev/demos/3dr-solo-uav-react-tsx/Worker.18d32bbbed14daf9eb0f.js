...
class DelegateReplayHandler extends DelegateHandler {
  ...
  async fetchData(startTime, endTime) {
    console.warn(`fetching ${startTime} -> ` + `${endTime} for datasource ${this.context.properties.dataSourceId}`);
    return this.context.doTemporalRequest(this.properties, startTime, endTime, this.status);
  }
...
class SweApiReplayContext extends SweApi_context {
  ...
  async doTemporalRequest(properties, startTime, endTime, status = {
    cancel: false
  }) {
    return new Promise(async (resolve, reject) => {
