...
(function (root, factory) {
    ...
    return function (config) {
        ...
        function initConfigDefaults() {
            ...
            config.numericalAccessFormPaths = config.numericalAccessFormPaths || [];
            ...
            if(config.removeAttrPrefix === undefined) {
        ...
        function checkAsNumericalValue(value,path) {
        	var displayAsNum = false;

        	for(var i=0;i < config.numericalAccessFormPaths.length;i++) {
				if((config.numericalAccessFormPaths[i] instanceof RegExp &&
                    config.numericalAccessFormPaths[i].test(path)) || (config.numericalAccessFormPaths[i] == value)) {
						displayAsNum = true;
						break;
				}
            }
        ...
        function parseDOMChildren( node, path,pos,parentResult ) {
            ...
            if(node.nodeType == DOMNodeTypes.ELEMENT_NODE) {
                ...
                if(nodePrefix!=null && nodePrefix!="" && config.prefix) {
                    result.__cnt++;
