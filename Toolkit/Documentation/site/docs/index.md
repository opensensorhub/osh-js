...
OpenSensorHub toolkit allows one to visualize data from OSH. It provides the necessary tools to build your own web application for monitoring your sensors.
...
configurable stylers and an advanced support for video (H264/MJPEG). It has been designed to integrate any map engines such as Lealfet, OpenLayer or Cesium.

Moreover, it offers support for SOS & SPS services, discovery functionnality, uses the HTTP or WebSocket API.
Several modules already exists to allow one to setup quickly an application such as Orientation, Chart, Video, Map etc..
