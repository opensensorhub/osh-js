...
define([
    ...
    function ImageDrapingPrimitive(options) {
        ...
        this._maxTerrainHeight = ImageDrapingPrimitive._defaultMaxTerrainHeight;
        this._minTerrainHeight = ImageDrapingPrimitive._defaultMinTerrainHeight;

        this._boundingSpheresKeys = [];
        this._boundingSpheres = [];

        // generate geometry instance from video footprint
    ...
    defineProperties(ImageDrapingPrimitive.prototype, {
         ...
         *
         * @memberof ImageDrapingPrimitive.prototype
         *
         * @type {Boolean}
         * @readonly
         *
         ...
         * true, the primitive will be rendered the next time that {@link ImageDrapingPrimitive#update}
         * is called.
         *
         * @memberof ImageDrapingPrimitive.prototype
         *
         ...
         * @memberof ImageDrapingPrimitive.prototype
         * @type {Promise.<ImageDrapingPrimitive>}
         * @readonly
         */
        readyPromise : {
            get : function() {
    ...
    ImageDrapingPrimitive._defaultMaxTerrainHeight = 9000.0;
    ImageDrapingPrimitive._defaultMinTerrainHeight = -100000.0;
    ...
    function setMinMaxTerrainHeights(primitive, rectangle, ellipsoid) {
        ...
        if (defined(xyLevel)) {
            ...
            var heights = ImageDrapingPrimitive._terrainHeights[key];
            if (defined(heights)) {
            ...
            if (defined(surfacePosition)) {
                ...
                minTerrainHeight = ImageDrapingPrimitive._defaultMinTerrainHeight;
            }
        }
    ...
    function createColorCommands(groundPrimitive, appearance, material, colorCommands) {
        ...
        var uniformMap = primitive._batchTable.getUniformMapCallback()(uniforms);*/
     ...
     * ImageDrapingPrimitive asynchronously.
     *
     * @returns {Promise} A promise that will resolve once the terrain heights have been loaded.
     *
     */
    ImageDrapingPrimitive.initializeTerrainHeights = function() {
        var initPromise = ImageDrapingPrimitive._initPromise;
        ...
        return ImageDrapingPrimitive._initPromise;
    };
    ...
    ImageDrapingPrimitive.prototype.update = function(frameState) {
        ...
        if (!ImageDrapingPrimitive._initialized) {
            //>>includeStart('debug', pragmas.debug);
            if (!this.asynchronous) {
                throw new DeveloperError('For synchronous ImageDrapingPrimitives, you must call ImageDrapingPrimitive.initializeTerrainHeights() and wait for the returned promise to resolve.');
            }
            //>>includeEnd('debug');

            ImageDrapingPrimitive.initializeTerrainHeights();
            return;
        }
        ...
        if (!defined(this._primitive)) {
            ...
            for (i = 0; i < length; ++i) {
                ...
                if (defined(instanceType) && defined(instanceType.createShadowVolume)) {
                    ...
                    //>>includeStart('debug', pragmas.debug);
                    throw new DeveloperError('Not all of the geometry instances have ImageDrapingPrimitive support.');
                    //>>includeEnd('debug');
                }
            }
     ...
     * @private
     */
    ImageDrapingPrimitive.prototype.getBoundingSphere = function(id) {
