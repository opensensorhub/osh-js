...
/**
 * Class used to visualize variable size image data
 *
 * @extends View
 * @example
 *
 import ImageView from 'core/ui/view/image/ImageView.js';

 let videoView = new ImageView({
  container: 'container',
  css: 'image',
  name: 'Static Images',
  showTime: true,
  ...
  layers: [
      new DataLayer({
        dataSourceId: imageDataSource.id
      })
  ]
});
 */
class ImageView extends View {
    ...
    constructor(properties) {
        ...
        if (isDefined(properties.showTime) && properties.showTime) {
            ...
            this.timeStamp.setAttribute("class", "image-time");
            document.getElementById(this.divId).appendChild(this.timeStamp);
        }

        // creates video tag element
        this.imgTag = document.createElement("img");
        this.imgTag.setAttribute("class", "static-image");
