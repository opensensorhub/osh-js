...
class OpenLayerView extends MapView {
    ...
    addMarker(properties) {
        ...
        if(isDefined(this.map) &&  this.map !== null) {
            let marker = new Point(transform([properties.location.x, properties.location.y], 'EPSG:4326', 'EPSG:900913'));
            ...
            if (this.first) {
                this.first = false;
                this.map.getView().setCenter(transform([properties.location.x, properties.location.y], 'EPSG:4326', 'EPSG:900913'));
