...
export const ExampleLeafletWithCustomBaseLayer = () => {
 // Similar to componentDidMount and componentDidUpdate:
  useEffect(() => {
        const dataSource = new SweJson("android-GPS", {
        	protocol: "ws",
    ...
    return <div style={{height:500 +'px'}} id="leaflet-single-baselayer-map"></div>;
}

<ExampleLeafletWithCustomBaseLayer/>
